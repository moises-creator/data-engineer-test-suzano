...
class LoadToBigQueryTG(TaskGroup):
    def __init__(self, group_id, tooltip="Criacao da tabela arquivos na camada bronze do BigQuery e Cloud Storage", **kwargs):
        ...
        load_bloomberg = GCSToBigQueryOperator(
            ...
            destination_project_dataset_table=f"{{params.project_id}}.{{params.dataset_id}}.bloomberg",
            source_format="CSV",
            skip_leading_rows=1,
        ...
        load_usd_cny = GCSToBigQueryOperator(
            ...
            destination_project_dataset_table=f"{{params.project_id}}.{{params.dataset_id}}.usd_cny",
            source_format="CSV",
            skip_leading_rows=1,
            write_disposition="WRITE_TRUNCATE",
            task_group=self
        ...
        load_china_index = GCSToBigQueryOperator(
            ...
            destination_project_dataset_table=f"{{params.project_id}}.{{params.dataset_id}}.chinese_caixin_services_index",
            source_format="CSV",
            skip_leading_rows=1,
            write_disposition="WRITE_TRUNCATE",
            task_group=self
